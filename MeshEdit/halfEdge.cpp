...
namespace HalfEdge
{
    ...
    HalfEdgeMesh::HalfEdgeMesh(int num, const std::vector<unsigned int>& indices)
        {
            //初始化顶点
        ...
        void HalfEdgeMesh::findSurroundingFaces(int index, std::vector<int>& surroundingFace)
        {
        ...
        void HalfEdgeMesh::findOuttingEdges(int index, std::vector<HeEdge*>& surroundingEdge)
        {
            HeVertex Targetvetex = m_vertices[index];
            Targetvetex = m_vertices[index];
            ...
            do {
                ...
                {
                    if ((edge->twin != nullptr))//避免边缘的边
                        surroundingEdge.push_back(edge->twin->next);
                    edge = edge->twin->next;
                }
            } while (edge != startEdge);
        }

        void HalfEdgeMesh::findSurroundingVertices(int index, std::vector<int>& surroundingVertices)
        {
            HeVertex Targetvetex = m_vertices[index];
            HeEdge* startEdge = Targetvetex.edge;
            HeEdge* edge = startEdge;
        ...
        void HalfEdgeMesh::addFace(const std::vector<unsigned int>& vertexIndices, int index)
        {
